 ...
 * Created by : Kavitha
 * Purpose    : To Add / remove rules from the knowledgebase to avoid duplicate records into system. 
 */
...
public class DuplicationHelper {
    ...
    /** The process entity. */
    private final ProcessEntity processEntity = ProcessEntity.getInstance();

    /**
     * Instantiates a new duplication helper.
     */
    private DuplicationHelper() {
	putil = new PropertiesUtil("ruleengine");
     ...
     */
    public static DuplicationHelper getInstance() {
	if (dupImpl == null) {
	    dupImpl = new DuplicationHelper();
	}
	return dupImpl;
    }
     ...
     * @return the string
     */
    public String generateRuleName(Storable storable) {
	return storable.getSite() + "_" + storable.getEntityType() + "_"
		+ storable.getId();
    }
     ...
     * @return the string
     */
    public String generateRuleName(Map map) {
	log.debug("generateRuleName:" + map);
     ...
     * @return the string
     */
    public String generateRule(String domainName, Storable storable,
	...
	try {
	    ...
	    throw new GenericException("ERR-RE-0006",
	...
	try {
	    String fileDir = putil.getProperty("rulepath") + domainName + "/"
		    + "unique/";
	    String filePath = fileDir + putil.getProperty("duplicatefile");
     ...
     */
    public HashMap getRuleData(Map<String, Object> dataMap, String uniqueDetails) {
     ...
     * @return the string
     */
    public String generateRule(Map<String, Object> ruleData) {
	log.methodEntry("generateRule entry");
	String validRule = "";
	try {
	    Map whenRuleData = (Map) ruleData.get("when");
	    Map<String, Object> orUniqueField = (Map) whenRuleData.get("OR");
	    Map<String, Object> andUniqueField = (Map) whenRuleData.get("AND");
	    ...
	    validRule = giveRule(formRule(ruleName, when,
		    thenFormation("unique", ruleName, orUniqueField), ruleData));
	    ...
	    throw new GenericException("ERR-RE-0011",
		    "DuplicationHelper.getRuledata failure:" + ex.getMessage(),
		    "Input is uniquejson string", ex);
	}
	log.methodExit("generateRule exit.");
     ...
     * @return the string
     */
    public String thenFormation(String type, String ruleName,
	...
	try {
	    ...
	    throw new GenericException("ERR-RE-0012",
     ...
     * @return true, if successful
     * 
     *         private boolean addRule(String domainName, Storable storable,
     *         String uniqueDetails) { log.methodEntry("addRule entry."); try {
     ...
     * @return true, if successful
     * 
     *         public boolean removeRule(String domainName, String entity,
     *         String id) { log.methodEntry("removeRule entry."); try {
     ...
     * @return true, if successful
     * 
     *         public boolean removeRule(String domainName, Storable storable) {
     *         log.methodEntry("removeRule entry."); try { List<String>
     ...
     *         catch (Exception ex) {
     *         log.error("In Overall EX : RuleEngineImpl.generateForDuplication:"
     *         + ex.getMessage()); // throw new //
     *         GenericException("ERR-RE-0015"
     ...
     * @return true, if successful
     * 
     *         public boolean generateForDuplication(String domainName,
     *         Collections<Storable> collections, String uniqueDetails) {
     *         log.methodEntry("generateForDuplication entry"); //try{
     *         if(collections.getCollection().size() == 0){ return false; } for
     *         (Storable storable : collections.getCollection()) { try {
     ...
     * @return the list
     * 
     *         public List<String> readRules(String filePath) {
     ...
     *         log.debug("After ReadKnowledge for remove.");
     * 
     *         // to add the rule rules.add(dupHelper.generateRule(ruleData));
     *         readKnowlegeBase("addrule", rules);
