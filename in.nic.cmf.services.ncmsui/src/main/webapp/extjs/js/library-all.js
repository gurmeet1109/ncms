...
Ext.define('Ext.ux.DataView.DragSelector', {
    ...
    /**
     * @private
     * Called when the attached DataView is rendered. This sets up the 
     * DragTracker instance that will be used
    ...
    onRender: function () {
         ...
         * The DragTracker attached to this instance. Note that the 4 on* 
         * functions are called in the scope of the 
         * DragTracker ('this' refers to the DragTracker inside those 
         * functions), so we pass a reference to the DragSelector 
         * so that we can call this class's functions.
         */
...
Ext.define('Ext.ux.tree.TreeEditing', {
    ...
    getEditingContext: function (record, columnHeader) {
        var me = this,
            grid = me.grid,
            store = grid.store,
            rowIdx, colIdx, view = grid.getView(),
...
Ext.override(Ext.form.field.HtmlEditor, {
    getSubTplData: function () {
        ...
        while (result.iframeSrc.indexOf('"') >= 0) {
            result.iframeSrc = result.iframeSrc.replace('"', "'");
        }
        return result;
    }
});
 ...
 * Overriden method to check authentication before triggering loading data
 */
 ...
 * applies customized validation types for NIC cms
 */

Ext.apply(Ext.form.field.VTypes, {
    alpha: function (val, field) {
        return /^([\u0900-\u0D7Fa-zA-Z ]+)$/.test(val);
    },
    alphaMask: /^([\u0900-\u0D7Fa-zA-Z ]+)$/,
    alphaText: 'This field should only contain letters',

    alphanumStrict: function (val, field) {
        return /^([\u0900-\u0D7Fa-zA-Z0-9 _-]+)$/.test(val);
    },
    alphanumStrictMask: /^([\u0900-\u0D7Fa-zA-Z0-9 _-]+)$/,
    alphanumStrictText: 'This field should only contain letters, numbers and _ -',
    alphanumSearch: function (val, field) {
        var ft = /^([\u0900-\u0D7Fa-zA-Z0-9 ._-]+)$/.test(val);
        if (!ft) return false;
        if(val.length >= 1) {
	    var pattern = /[\.\-\_]/gi;
            if(val.charAt(0).match(pattern)) return false;
	    else return true;
	}
    },
    alphanumSearchMask: /^([\u0900-\u0D7Fa-zA-Z0-9 ._-]+)$/,
    alphanumSearchText: 'This field should only contain letters, numbers and _-.',
    alphanumMid: function (val, field) {
        return /^([\u0900-\u0D7Fa-z.:;@#,()\/"'A-Z0-9 _-]+)$/.test(val);
    },
    alphanumMidMask: /^([\u0900-\u0D7Fa-z.:;@#,()\/"'A-Z0-9 _-]+)$/,
    alphanumMidText: 'This field should only contain oletters, numbers and allowed special characters [.:;@#,()"\'/ _-]',
    passwordVType: function (val, field) { 
      //  return /^(?=.*[A-Za-z])(?=.*[0-9])(?!.*\s).{8,10}$/.test(val);
        return /^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[.:;@#,()\/"'_-])(?!.*[~`?<>+=*{}$/&^%!|])(?!.*\s).{8,10}$/.test(val);
    },
    // passwordVTypeMask: /^(?=.*\d)(?=.*[a-zA-Z])(?=.*[.:;@#_-()/"'])(?!.*\s).{8,10}$/,
    passwordVTypeText: 'This field should contain atleast one number, one character, one special character, one character in upper-case and it should be 8-10 characters in length',

    alphanumLoose: function (val, field) {
        return /^([\u0900-\u0D7Fa-z.:;@#!$%^&*{},()\\/"'A-Z0-9 _-]+)$/.test(val);
    },
    alphanumLooseMask: /^([\u0900-\u0D7Fa-z.:;@#!$%^&*{},()\\/"'A-Z0-9 _-]+)$/,
    alphanumLooseText: 'This field should only contain letters, numbers and special characters',

    alphanumeric: function (val, field) {
        return /^[\u0900-\u0D7F0-9a-zA-Z ]+$/.test(val);
    },
    alphanumericMask: /^[\u0900-\u0D7F0-9a-zA-Z ]+$/,
    alphanumericText: 'This field should only contain letters and numbers',

    searchText: function (val, field) {
        var ft = /^([\u0900-\u0D7Fa-z.:;@#,\/"A-Z0-9 _-]+)$/.test(val);
        if (!ft) return false;
        if (val.length >= 1) {
            var pattern = /[\.\,\#]/gi;
            if (val.charAt(0).match(pattern)) return false;
    ...
    searchTextMask: /^([\u0900-\u0D7Fa-z.:;@#,\/"A-Z0-9 _-]+)$/,
    searchTextText: 'This field should only contain letters, numbers and some special characters',
    ...
    floatStrict: function (val, field) {
        return /^([0-9.]+)$/.test(val);
    },
    floatStrictMask: /^([0-9.]+)$/,
    floatStrictText: 'This field should contain only numbers and .',

    filename: function (val, field) {
        var regArray = val.split(".");
        if (regArray.length == 2) {
            if (!regArray[1]) return false;
            if (regArray[0].length >= 1 && regArray[1].length >= 2 && regArray[1].length <= 4) return true
        } else if (regArray.length == 3) {
            if (!regArray[2]) return false;
            if (regArray[0].length >= 1 && regArray[1].length >= 1 && regArray[2].length >= 2 && regArray[2].length <= 4) return true;
        } else return false;
    },
    ...
    prefix: function (val, field) {
        return /^[\u0900-\u0D7Fa-zA-Z ]{0,6}$/.test(val);
    },
    prefixMask: /^[\u0900-\u0D7Fa-zA-Z ]{0,6}$/,
...
/**
 * Validator function, checks the given string is a valid ISBN number or not
 * @param isbn
 * @returns {Boolean}
 */
